...
func (suite *OauthTestSuite) TestAuthenticateRollingRefreshToken() {
	...
	).Order("updated_at").Find(&refreshTokens).Error
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), "test_token_1", refreshTokens[0].Token)
	assert.True(
		suite.T(),
		math.Abs(float64(now1.Unix()+int64(suite.cfg.Oauth.RefreshTokenLifetime)-refreshTokens[0].ExpiresAt.Unix())) < 5.0,
	)
	assert.Equal(suite.T(), "test_token_2", refreshTokens[1].Token)
	assert.True(
		suite.T(),
		math.Abs(float64(testRefreshTokens[1].ExpiresAt.Unix()-refreshTokens[1].ExpiresAt.Unix())) < 5.0,
	)
	assert.Equal(suite.T(), "test_token_3", refreshTokens[2].Token)
	assert.True(
		suite.T(),
		math.Abs(float64(testRefreshTokens[2].ExpiresAt.Unix()-refreshTokens[2].ExpiresAt.Unix())) < 5.0,
	)
	...
	assert.True(
		suite.T(),
		math.Abs(float64(now2.Unix()+int64(suite.cfg.Oauth.RefreshTokenLifetime)-refreshTokens[1].ExpiresAt.Unix())) < 5.0,
	)
	assert.Equal(suite.T(), "test_token_3", refreshTokens[2].Token)
	assert.True(
		suite.T(),
	...
	assert.True(
		suite.T(),
		math.Abs(float64(now3.Unix()+int64(suite.cfg.Oauth.RefreshTokenLifetime)-refreshTokens[2].ExpiresAt.Unix())) < 5.0,
	)
}
...
func (suite *OauthTestSuite) TestClearUserTokens() {
	...
	testUserSession = &session.UserSession{
		ClientID:     suite.clients[0].ClientKey,
